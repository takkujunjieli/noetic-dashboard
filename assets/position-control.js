...
export async function mountPositionControl(host,{c,cases,readonly,onBind,onSync,onRule,onDirty}){
 ...
 function paint(){
  ...
  host.innerHTML=`<h3>关联 Portfolio 实际持仓</h3><p class="hint">复用同一份本地持仓文件。这里的刷新不会调用券商 MCP；券商刷新流程仍由 Portfolio 原有流程负责。</p>
   ...
   <p class="hint">只列出 ${esc(c.linkedSymbols?.join("、")||c.symbol||"全部标的")} 及其完整期权合约。整仓跟随会同步后续增减仓；固定数量保留指定份额。固定份额减仓归属不明时暂停同步。</p>
   <div class="table-wrap"><table class="position-picker"><thead><tr><th>归属</th><th>持仓 / 账户</th><th>账户数量 / 他用</th><th>方式 / 分配量</th></tr></thead><tbody>${source.rows.filter(r=>(c.linkedSymbols?.length?c.linkedSymbols.includes(r.underlying):!c.symbol||r.underlying===c.symbol.toUpperCase())&&(!account||r.account===account)).map(r=>{
