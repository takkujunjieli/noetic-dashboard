...
test('link multiple underlyings without writing sources; duplicates and occupied positions protected',()=>{
 ...
 assert.equal(c.nodes.positions.data.positions.length,2);assert.equal(c.nodes.hypothesis.data.rationale,'because');assert.equal(c.nodes.hypothesis.data.invalidation,'unless');assert.equal(c.design.plans.length,0);
 ...
 const other={...catalog.items[0],name:'Other'},d=linkSource(other,catalog,[c]);assert.equal(d.nodes.positions.data.positions.length,0);assert.equal(d.sourceLink.warnings.length,2);
});
test('local thesis policy overrides file and missing portfolio does not invent positions',()=>{
 ...
 assert.equal(c.title,'Local');assert.equal(c.symbol,'B');assert.equal(c.nodes.positions.data.positions.length,0);assert.ok(catalog.error);
});
